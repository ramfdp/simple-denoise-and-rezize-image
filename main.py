...
def ImageRezize(images_folder, size=(800, 600)):
    images = [os.path.join(images_folder, f) for f in os.listdir(images_folder) if f.endswith(('.jpg', '.png', '.jpeg')) and '_resized' not in f]
    resized_paths = []
...
def denoise_and_save(image_path, output_folder):
    ...
    bilateral_output = os.path.join(output_folder, f"{name}_bilateral_denoised{ext}")
    ...
    cv2.imwrite(bilateral_output, cv2.cvtColor(bilateral_denoised_image, cv2.COLOR_RGB2BGR))
    cv2.imwrite(nlm_output, cv2.cvtColor(nlm_denoised_image, cv2.COLOR_RGB2BGR))
    
    print(f"Denoised images saved: {median_output}, {bilateral_output}, {nlm_output}")
